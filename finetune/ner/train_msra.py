...
from fastNLP.io import MsraNERLoader, MsraNERPipe
...
demo = False
if demo:
    cache_fn = f"dataset/cache/demo-{model_name}-{args.max_length}"
else:
    cache_fn = f"dataset/cache/msra-{model_name}-{args.max_length}"
print(cache_fn)
...
def load_data(tokenizer_name):
    ...
    db = MsraNERPipe().process_from_file(data_path)
...
if get_local_rank() == 0:
    ...
    os.makedirs("logs/msra", exist_ok=True)
    fitlog.set_log_dir("logs/msra")
...
tester = Tester(data=data_bundle.get_dataset("test"), metrics=metric, model=model,batch_size=args.batch_size)
callbacks_master = [
